...
namespace SSIS.DataService.SSISDB
{
    ...
    /// <summary>
    /// This file contains the source code for the ExecutionMessages routine.
    /// </summary>
    public partial class Service
    {
        #region Build SqlCommand

        private SqlCommand ExecutionMessages_BuildCommand(SqlConnection cnn, ExecutionMessagesInput input)
        {
...
SET @Count = @@ROWCOUNT;",
        ...
        private void ExecutionMessages_SetParameters(SqlCommand cmd, ExecutionMessagesOutput output)
        {
        ...
        private ExecutionMessagesResult ExecutionMessages_ResultData(SqlDataReader rdr)
        {
            return new ExecutionMessagesResult(
        ...
        private void ExecutionMessages_Execute(SqlCommand cmd, ExecutionMessagesOutput output)
        {
            using (SqlDataReader rdr = cmd.ExecuteReader())
            {
                output.ResultData = new List<ExecutionMessagesResult>();
                while(rdr.Read())
                {
                    output.ResultData.Add(ExecutionMessages_ResultData(rdr));
                }
                rdr.Close();
            }

            ExecutionMessages_SetParameters(cmd, output);
        }
        ...
        /// DB Routine: dbo.ExecutionMessages<br/>
        /// Author: Todd Zimmerman<br/>
        /// </summary>
        /// <param name="input">ExecutionMessagesInput instance.</param>
        /// <returns>Instance of ExecutionMessagesOutput</returns>
        public ExecutionMessagesOutput ExecutionMessages(ExecutionMessagesInput input)
        {
            ValidateInput(input, nameof(ExecutionMessages));
            ExecutionMessagesOutput output = new ExecutionMessagesOutput();
			if(sqlConnection != null)
            {
                using (SqlCommand cmd = ExecutionMessages_BuildCommand(sqlConnection, input))
                {
                    cmd.Transaction = sqlTransaction;
                    ExecutionMessages_Execute(cmd, output);
                }
                return output;
            }
            for(int idx=0; idx <= retryOptions.RetryIntervals.Count; idx++)
            {
                ...
                try
                {
                    ...
                    using (SqlCommand cmd = ExecutionMessages_BuildCommand(cnn, input))
                    {
                        cnn.Open();
						ExecutionMessages_Execute(cmd, output);
                        cnn.Close();
